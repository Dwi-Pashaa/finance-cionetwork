...
<div class="card cio-card">
    <div class="cio-card-header d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
        <div>
            <h3 class="cio-title">Data Pengeluaran</h3>
            <div class="cio-subtitle">Kelola biaya, pembayaran, vendor, dan biaya admin</div>
        </div>
        @can('tambah pengeluaran')
            <a href="{{ route('expense.create') }}" class="btn cio-btn cio-btn-primary">
        ...
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-3">
                ...
                <div class="input-group input-group-sm">
                    <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Vendor, kategori, catatan...">
        ...
        <table class="table card-table table-vcenter cio-table">
            ...
            <tbody>
                @forelse ($expenses as $item)
                    <tr>
                        <td class="text-center text-muted fw-semibold">{{ $loop->iteration + ($expenses->currentPage() - 1) * $expenses->perPage() }}</td>
                        <td>{{ $item->transaction_date->format('d M Y') }}</td>
                        <td><span class="badge bg-danger-lt">{{ $item->category?->name ?? '-' }}</span></td>
                        <td>
                            <div class="fw-semibold">{{ $item->payee ?? '-' }}</div>
                            <div class="small text-muted">{{ $item->description ?? '-' }}</div>
                        </td>
                        <td class="text-end">Rp {{ number_format($item->amount, 0, ',', '.') }}</td>
                        <td class="text-end">{{ $item->has_admin_fee ? 'Rp '.number_format($item->admin_fee_amount, 0, ',', '.') : '-' }}</td>
                        <td class="text-end cio-currency">Rp {{ number_format($item->total_amount, 0, ',', '.') }}</td>
                        <td class="text-center">
                            <div class="action-btn-group">
                                @can('edit pengeluaran')
                                    <a href="{{ route('expense.edit', $item->id) }}" class="cio-action-btn btn-edit" title="Edit Pengeluaran">
                                ...
                                @can('hapus pengeluaran')
                                    <button type="button" onclick="deleteExpense('{{ $item->id }}')" class="cio-action-btn btn-delete" title="Hapus Pengeluaran">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="cio-empty-state">Belum Ada Data Pengeluaran</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="cio-table-footer d-flex flex-column flex-md-row align-items-center justify-content-between gap-3">
        <p class="m-0 text-muted small">Menampilkan {{ $expenses->firstItem() ?? 0 }} - {{ $expenses->lastItem() ?? 0 }} dari {{ $expenses->total() }} data</p>
        <div class="m-0">{{ $expenses->links() }}</div>
    </div>
</div>
@endsection
...
@push('js')
<script>
    const EXPENSE_BASE = "{{ route('expense.index') }}";
    $("#sort").change(function() {
        const params = new URLSearchParams(window.location.search);
        params.set('sort', $(this).val());
        window.location.href = EXPENSE_BASE + '?' + params.toString();
    });
